...
const localModelEnvironmentSchema = z.object({
  ...
  LOCAL_LLM_REQUEST_TIMEOUT_MS: z.coerce
    .number()
    .int()
    .min(1_000)
    .max(120_000)
...
export function loadLocalModelConfig(
  ...
  return {
    ...
    apiKey: parsed.LOCAL_LLM_API_KEY,
    requestTimeoutMs: parsed.LOCAL_LLM_REQUEST_TIMEOUT_MS,
...
export function createLocalGemmaModel(
  ...
  const provider = createOpenAICompatible({
    ...
    fetch: createLlamaCppFetch(config.requestTimeoutMs, fetchImplementation),
