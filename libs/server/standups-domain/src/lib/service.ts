...
export class StandupService {
  ...
  async createRosterMember(
    ...
    await this.getTeam();
    return this.repository.createMember(this.scope, input, now);
  }
  ...
  async getTeamPulse(
    ...
    const blockersByMember = new Map<string, typeof openBlockers>();
    ...
    const selectedDay = calculateDay(members, selectedStandups, openBlockers);
    const previousDay = await this.getPulseDay(
      shiftIsoDate(date, -1),
      members.length,
    );
    const trend = await Promise.all(
      dates.map(async (trendDate) => {
        const day =
          trendDate === date
            ? selectedDay
            : await this.getPulseDay(trendDate, members.length);
    ...
    return {
      ...
      blockers: openBlockers.map((blocker) => {
        const member = members.find(
          (candidate) => candidate.id === blocker.memberId,
        );
        if (!member) {
          throw new StandupDomainError(
            'MEMBER_NOT_FOUND',
            'Blocker owner is not in the active roster',
          );
        }
        return {
          id: blocker.id,
  ...
  async generateDailySnapshot(
    ...
    const day = await this.getPulseDay(date, members.length, false);
  ...
  private async getPulseDay(
    ...
    return calculateDayFromCount(roster, standups.length, blockers.length);
  }
}
