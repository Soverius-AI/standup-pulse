...
export class SqliteStandupRepository implements StandupRepository {
  ...
  async saveStandupWithBlockers(
    ...
    return this.db.transaction((tx) => {
      if (command.source.sourceEventId) {
        const duplicate = tx
          .select()
          .from(standups)
          .where(
            and(
              ...
              eq(standups.sourceEventId, command.source.sourceEventId),
            ),
          )
          .get();
        if (duplicate) {
      ...
      const standupRow = {
        ...
        sourceEventId: command.source.sourceEventId ?? null,
      };
  ...
  async listBlockers(
    ...
    return this.db
      ...
      .from(blockers)
      .where(
        and(eq(blockers.teamId, scope.teamId), eq(blockers.status, status)),
      )
      .orderBy(desc(blockers.openedAt))
      .limit(limit)
      .all()
      .map(mapBlocker);
  }
  ...
  async claimDelivery(
    ...
    const result = this.db
      ...
      .onConflictDoNothing({ target: deliveryOutbox.idempotencyKey })
      .run();
    let row = this.db
      .select()
      .from(deliveryOutbox)
      .where(eq(deliveryOutbox.idempotencyKey, idempotencyKey))
      .get();
    if (!row) throw new Error('Delivery claim was not persisted');
    let claimed = result.changes === 1;
    if (!claimed && row.status === 'failed' && row.availableAt <= now) {
      this.db
        ...
        .where(eq(deliveryOutbox.idempotencyKey, idempotencyKey))
        .run();
      row = { ...row, status: 'pending' as const, lastError: null };
      claimed = true;
    }
    return {
      claimed,
...
function mapStandup(row: typeof standups.$inferSelect): StandupRecord {
  const { sourceMessageId, sourceEventId, ...required } = row;
  return {
    ...required,
    workDate: IsoDateSchema.parse(row.workDate),
...
function mapSnapshot(
  ...
  return { ...row, workDate: IsoDateSchema.parse(row.workDate) };
}
